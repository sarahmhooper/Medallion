...
class ShearX(DauphinTransform):
    ...
    def transform(self, img, label=None, **kwargs):
        ...
        if label:
            return (
                self._shearx(img, degree),
                {key: self._shearx(label[key], degree) for key in label.keys()},
            )
        else:
            return self._shearx(img, degree)

    def _shearx(self, img, degree):
        if isinstance(img, list):
            return [
                i.transform(i.size, Image.AFFINE, (1, degree, 0, 0, 1, 0)) for i in img
            ]
        else:
            return img.transform(img.size, Image.AFFINE, (1, degree, 0, 0, 1, 0))
